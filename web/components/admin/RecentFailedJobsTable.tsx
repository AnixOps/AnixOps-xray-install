...
export interface RecentFailedJobRow {
  id: string;
  name: string;
  state?: string;
...
interface RecentFailedJobsTableProps {
  rows: RecentFailedJobRow[];
  onOpenRental?: (rentalId: string) => void;
}

export function RecentFailedJobsTable({ rows, onOpenRental }: RecentFailedJobsTableProps) {
  const [sorting, setSorting] = useState<SortingState>([{ id: "timestamp", desc: true }]);

  const columns = useMemo<ColumnDef<RecentFailedJobRow>[]>(
    () => [
      {
        id: "name",
      ...
      },
      {
        ...
        header: "Data",
        cell: ({ row }) => {
          const dataText = safeStringify(row.original.data);
          return (
            <div className="max-w-[320px] truncate font-mono text-[11px] text-muted-foreground" title={dataText}>
              {dataText}
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="max-w-[360px] truncate text-[11px] text-muted-foreground" title={stack}>
              {stack}
            </div>
          );
        },
  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b p-4">
        <div>
          ...
          <div className="mt-1 text-sm text-muted-foreground">
            Provision queue failures, payloads, and stacktrace previews.
          </div>
        </div>
      ...
      {rows.length === 0 ? (
        <div className="p-4 text-sm text-muted-foreground">No failed jobs.</div>
      ) : (
                  ...
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      ...
                      className={cn(
                        ...
                        cell.column.id === "reason" && "min-w-[300px]",
                        cell.column.id === "data" && "min-w-[320px]",
                        cell.column.id === "stacktrace" && "min-w-[360px]",
...
function formatJobTime(job: RecentFailedJobRow) {
